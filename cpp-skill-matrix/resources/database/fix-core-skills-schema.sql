-- Fix Core Skills Schema - Drop and recreate with correct column types
-- This migration fixes the schema mismatch between database and code

PRINT 'Dropping existing core skills tables...';

-- Drop in correct order due to foreign keys
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[core_skill_assessments]') AND type in (N'U'))
BEGIN
    DROP TABLE [dbo].[core_skill_assessments];
    PRINT 'Dropped table: core_skill_assessments';
END
GO

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[core_skills]') AND type in (N'U'))
BEGIN
    DROP TABLE [dbo].[core_skills];
    PRINT 'Dropped table: core_skills';
END
GO

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[core_skill_categories]') AND type in (N'U'))
BEGIN
    DROP TABLE [dbo].[core_skill_categories];
    PRINT 'Dropped table: core_skill_categories';
END
GO

PRINT 'Creating tables with correct schema...';

-- Core skills categories table (NVARCHAR IDs)
...
-- Core skills table (NVARCHAR IDs)
...
-- Core skill assessments table (NVARCHAR category_id and skill_id)
...
PRINT 'Inserted core skill categories';
GO
...
PRINT 'Inserted core skills';
GO

PRINT '';
PRINT '✅ Schema fix complete! Core skills tables now match the code.';
GO
